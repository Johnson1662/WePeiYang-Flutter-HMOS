...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  int _safeTabIndex(int index) => index.clamp(0, _tabs.length - 1).toInt();
  ...
  TabController _createTabController(int initialIndex) {
    ...
    controller = TabController(
      length: _tabs.length,
      vsync: this,
      initialIndex: initialIndex.clamp(0, _tabs.length - 1).toInt(),
    )..addListener(() {
  ...
  void _handleXiaotianTabVisibilityChanged() {
    final currentTab = _tabs[_safeTabIndex(_currentIndex)].id;
  ...
  Widget build(BuildContext context) {
    final currentTabId = _tabs[_safeTabIndex(_currentIndex)].id;
